...
public class SoundDetector extends Thread {
    ...
    private int maxIndex = 0,maxIndexFFT = 0,sample_rate,lowFreq,upFreq;
    ...
    private TextView last_detected_TV,diff_detected_TV,fft_detected_TV;
    private FFT fft;
    private PlayManager mPlayManager;
    private int buffer_Size = 1024 * 2;

    public SoundDetector(String name,TextView last_detected_t,TextView diff_detected,TextView fft_detected_tv,int sample_rate,String f_name) {
        super(name);
        ...
        this.fft_detected_TV = fft_detected_tv;
        ...
        String dev = f_name.substring(0,1);
        if(dev.equals("A")){
            lowFreq = 6500;
            upFreq = 7500;
        } else if(dev.equals("B")){
            lowFreq = 2500;
            upFreq = 3500;
        } else if(dev.equals("C")){
            lowFreq = 5500;
            upFreq = 6500;
        } else if(dev.equals("D")){
            lowFreq = 4500;
            upFreq = 5500;
        } else {
            lowFreq = 2500;
        ...
        Log.e("Dev Freq ","Dev : "+dev+" lowFreq = "+lowFreq+" upFreq = "+upFreq);

        mPlayManager = new PlayManager(9, f_name,sample_rate,1000);
    ...
    public void run() {
        ...
        while (isRecording) {
            ...
            if(result[maxIndex] > 5000 && ((((maxIndexFFT*sample_rate)/512) > lowFreq) && (((maxIndexFFT*sample_rate)/512) < upFreq ))) {
                ...
                final double temp = result[maxIndex];
                handler.post(new Runnable(){
                    public void run(){
                            last_detected_TV.setText(String.format("%.2f", t_last_det));
                            last_detected_TV.setTextColor(Color.RED);
                            fft_detected_TV.setText(String.format("%.2f", (float) (maxIndexFFT*sample_rate)/512));
                            fft_detected_TV.setTextColor(Color.RED);
                            diff_detected_TV.setText(String.format("%.2f", temp));
